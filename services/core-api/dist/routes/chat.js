...
router.get('/history', auth_js_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const { data: conversation, error: convError } = await supabase_js_1.supabase
            .from('conversations')
            .select('*')
        ...
        const { data: messages, error: messagesError } = await supabase_js_1.supabase
            .from('messages')
            .select('*')
 ...
 */
router.post('/message', auth_js_1.authMiddleware, async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ error: 'User not authenticated' });
    }
    ...
    try {
        ...
        if (!conversationId) {
            const { data: newConversation, error: convError } = await supabase_js_1.supabase
                .from('conversations')
                .insert([
                {
                    user_id: req.user.id,
            ...
            const { data: existingConv, error: checkError } = await supabase_js_1.supabase
                .from('conversations')
                .select('id')
        ...
        const { data: userMessage, error: userMsgError } = await supabase_js_1.supabase
            .from('messages')
            .insert([
            {
        ...
        const targetRoles = userProfile?.target_roles || [];
        // Fetch last 10 messages from this conversation for context
        const { data: recentMessages } = await supabase_js_1.supabase
            .from('messages')
            .select('role, content')
        ...
        try {
            const aiResponse = await ai_client_js_1.aiClient.generateResponse({ skills, target_roles: targetRoles }, messageHistory, content.trim());
        ...
        const { data: assistantMessage, error: assistantMsgError } = await supabase_js_1.supabase
            .from('messages')
            .insert([
            {
                conversation_id: conversationId,
