...
router.get('/', auth_js_1.authMiddleware, async (req, res) => {
    ...
    try {
        const { data: profile, error } = await supabase_js_1.supabase
            .from('user_profiles')
            .select('*')
 ...
 * }
 */
router.put('/', auth_js_1.authMiddleware, async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ error: 'User not authenticated' });
    }
    const { full_name, headline, location, about, long_term_vision, skills, target_roles, work_style, industries, goal, cv_url, } = req.body;
    ...
    try {
        ...
        if (target_roles !== undefined)
            updateData.target_roles = target_roles;
        if (work_style !== undefined)
            updateData.work_style = work_style;
 ...
 * }
 */
router.put('/onboarding', auth_js_1.authMiddleware, async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ error: 'User not authenticated' });
    }
    const { full_name, headline, location, skills, target_roles, work_style, industries, goal, long_term_vision, cv_url, } = req.body;
    // Validate work_style enum if provided
    if (work_style && !['Remote', 'Hybrid', 'On-site'].includes(work_style)) {
        return res.status(400).json({
            error: 'Invalid work_style. Must be one of: Remote, Hybrid, On-site',
    ...
    try {
        ...
        const userEmail = await (0, stripeHelpers_js_1.getUserEmail)(req.user.id);
        await (0, stripeHelpers_js_1.ensureStripeCustomer)(req.user.id, userEmail, full_name);
        // Build update object with only provided fields
        const updateData = {
