...
exports.fetchSkillsGapForJob = fetchSkillsGapForJob;
...
function generateSkillsAnalysis(userSkills, jobRequirements) {
    const normalizedUserSkills = userSkills.map((s) => s.toLowerCase().trim());
    return jobRequirements.map((requirement) => {
        const normalizedReq = requirement.toLowerCase().trim();
        const isMatched = normalizedUserSkills.some((userSkill) => userSkill.includes(normalizedReq) || normalizedReq.includes(userSkill));
        return {
            name: requirement,
...
function calculateMatchScore(userSkills, jobRequirements) {
    ...
    const matchedCount = jobRequirements.filter((requirement) => {
        const normalizedReq = requirement.toLowerCase().trim();
        return normalizedUserSkills.some((userSkill) => userSkill.includes(normalizedReq) || normalizedReq.includes(userSkill));
    }).length;
    return Math.round((matchedCount / jobRequirements.length) * 100);
}
const sanitizeFilterValue = (value) => value.replace(/[%_]/g, '').trim();
async function fetchSkillsGapForJob(userSkills = [], jobRequirements = []) {
    if (!jobRequirements.length) {
        return undefined;
    }
    const response = await ai_client_js_1.aiClient.getSkillGap(userSkills, jobRequirements);
    if (!response) {
        return undefined;
    }
    return {
...
router.post('/find-matches', auth_js_1.authMiddleware, (0, validation_js_1.validateRequest)(findMatchesSchema), async (req, res, next) => {
    try {
        ...
        const { error: userError } = await supabase_js_1.supabase
            .from('users')
            .select('*')
            .eq('id', userId)
            .single();
        ...
        let jobQuery = supabase_js_1.supabase.from('jobs').select('*');
        if (filters?.workType) {
        ...
        const aiHealthy = await ai_client_js_1.aiClient.healthCheck();
        let jobsWithScores;
        if (aiHealthy && jobs.length > 0) {
            // Use AI matching for intelligent results
            try {
                const matches = await ai_client_js_1.aiClient.generateMatches({
                    profile: {
                        user_id: userId,
                ...
                const matchesWithSkillGaps = await Promise.all(matches.map(async (match) => {
                    const job = jobs.find((j) => j.id === match.job_id);
                    const skillsAnalysis = generateSkillsAnalysis(userProfile.skills || [], job?.requirements || []);
                    // Fetch skill gap analysis from AI Engine
                    const skillsGap = await fetchSkillsGapForJob(userProfile.skills || [], job?.requirements || []);
                    return {
                        ...job,
                ...
                const jobsWithSkillGaps = await Promise.all(jobs.map(async (job) => {
                    ...
                    const skillsGap = await fetchSkillsGapForJob(userSkills, job.requirements || []);
                    return {
                        ...job,
                        matchScore,
...
const searchSchema = zod_1.z.object({
    ...
    location: zod_1.z.string().trim().max(100, 'Location too long').optional(),
});
