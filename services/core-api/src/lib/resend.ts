 ...
 */
function baseTemplate(content: string, unsubscribeToken?: string): string {
  return `
    <!DOCTYPE html>
    <html>
      <head>
 ...
 * Welcome template
 */
export function generateWelcomeTemplate(name: string): string {
  const content = `
 ...
 */
export function generateCardExpiringTemplate(
  name: string,
  brand: string,
  lastFour: string,
  expiryMonth: number,
 ...
 */
export function generateSubscriptionConfirmationTemplate(
  name: string,
  planName: string,
  price: number,
  billingCycle: 'monthly' | 'yearly'
 ...
 */
export function generateApplicationStatusTemplate(
  name: string,
  jobTitle: string,
  company: string,
  status: 'applied' | 'reviewing' | 'shortlisted' | 'rejected' | 'offer'
