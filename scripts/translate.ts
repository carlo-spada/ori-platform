...
if (!DEEPL_API_KEY) {
  console.error(`${colors.red}❌ Error: DEEPL_API_KEY environment variable is required${colors.reset}`)
...
function notifyError(error: Error | unknown, context: string): void {
  ...
  if (errorMessage.includes('Authorization failure') || errorMessage.includes('403')) {
    console.error(`\n${colors.red}❌ API Key Issue Detected${colors.reset}`)
    console.error(`   • Key type: ${isFreeKey ? 'FREE (ends with :fx)' : 'PRO'}`)
...
async function translateText(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (options.verbose) {
        console.log(`${colors.yellow}   ⏳ Retry ${attempt}/${retries} after ${delay}ms${colors.reset}`)
      }
...
async function processNamespace(
  ...
  if (!sourceContent) {
    console.log(`${colors.yellow}   ⚠️  Source not found: ${sourcePath}${colors.reset}`)
    stats.skipped++
    return
  }
  ...
  // Check-only mode
  if (options.check) {
    console.log(`${colors.yellow}   ⚠️  Missing ${missingKeys.length} keys${colors.reset}`)
    if (options.verbose) {
      missingKeys.slice(0, 5).forEach(key => {
        console.log(`${colors.dim}      - ${key}${colors.reset}`)
      })
      if (missingKeys.length > 5) {
        console.log(`${colors.dim}      ... and ${missingKeys.length - 5} more${colors.reset}`)
      }
    }
    return
  }
  ...
  // Dry-run mode
  if (options['dry-run']) {
    console.log(`${colors.blue}   🔍 Would translate ${missingKeys.length} keys${colors.reset}`)
    return
  }

  // Translate missing keys
  console.log(`${colors.cyan}   📝 Translating ${missingKeys.length} keys...${colors.reset}`)
  ...
  console.log(`${status}   ${icon} Translated: ${translated}, Failed: ${failed}${colors.reset}`)
}
...
async function main() {
  ...
  try {
    ...
    if (usage.character) {
      ...
      if (percent >= 90) {
        console.warn(`${colors.red}⚠️  WARNING: Only ${remaining.toLocaleString()} characters remaining!${colors.reset}`)
      } else if (percent >= 75) {
        console.warn(`${colors.yellow}📊 Note: ${percent}% of quota used${colors.reset}`)
      }
    }
    ...
    // Validate selections
    const invalidNamespaces = namespacesToProcess.filter(ns => !ALL_NAMESPACES.includes(ns))
    if (invalidNamespaces.length > 0) {
      console.error(`${colors.red}❌ Invalid namespaces: ${invalidNamespaces.join(', ')}${colors.reset}`)
      console.error(`Available: ${ALL_NAMESPACES.join(', ')}`)
      process.exit(1)
    }

    const invalidLanguages = languagesToProcess.filter(lang => !(lang in TARGET_LANGUAGES))
    if (invalidLanguages.length > 0) {
      console.error(`${colors.red}❌ Invalid languages: ${invalidLanguages.join(', ')}${colors.reset}`)
    ...
    if (options.check) {
      ...
      console.log(`${colors.bright}${colors.green}✅ Translation Complete${colors.reset}`)
      console.log(`Files processed: ${stats.checked}`)
      console.log(`Keys translated: ${stats.translated}`)

      if (stats.failed > 0) {
        console.log(`${colors.yellow}Keys failed: ${stats.failed}${colors.reset}`)
        console.log(`${colors.dim}See ${FAILED_LOG_FILE} for details${colors.reset}`)
      }
      ...
      if (finalUsage.character && usage.character) {
        ...
        console.log(`Session usage: ${sessionUsage.toLocaleString()} characters`)
      }
    }
