...
const options = {
  namespace:
    args.find((arg) => arg.startsWith('--namespace='))?.split('=')[1] || null,
  language:
    args.find((arg) => arg.startsWith('--language='))?.split('=')[1] || null,
 ...
 */
function preservePatterns(text: string): {
  processed: string
...
async function translateText(
  ...
  try {
    ...
    if (errorMessage.includes('Too many requests') && retries > 0) {
      console.log(
        `${colors.yellow}⚠️  Rate limited, waiting 2 seconds...${colors.reset}`,
      )
      await delay(2000)
...
async function translateObject(
  ...
  if (Array.isArray(obj)) {
    ...
    for (const item of obj) {
      translated.push(
        await translateObject(item, targetLang, parentKey, depth + 1),
      )
    }
    return translated
  }
...
async function extractTranslationKeys(): Promise<Set<string>> {
  ...
  for (const file of files) {
    ...
    const nsMatches = content.matchAll(
...
async function translateNamespace(
  ...
  if (!(await fileExists(sourcePath))) {
    console.log(
      `${colors.yellow}⚠️  Source file not found: ${sourcePath}${colors.reset}`,
    )
    return
  }
  ...
  if (!options.force && (await fileExists(targetPath))) {
    ...
    if (sourceKeys === targetKeys) {
      console.log(
        `${colors.green}✓${colors.reset} Already translated: ${targetPath}`,
      )
      return
    }
  }
  ...
  console.log(
    `${colors.blue}→${colors.reset} Translating ${namespace} to ${targetLang}...`,
  )
  ...
  if (!options.force && (await fileExists(targetPath))) {
    ...
    finalContent = mergeTranslations(
      existingJson,
...
async function main() {
  ...
  try {
    ...
    if (usage.character) {
      ...
      console.log(
        `📊 Usage: ${usage.character.count.toLocaleString()} / ${usage.character.limit.toLocaleString()} characters (${usagePercent}%)`,
      )

      if (usagePercent >= 90) {
        console.warn(
          `${colors.red}⚠️  Warning: You are near your character limit!${colors.reset}`,
        )
      }
    }
    ...
    console.log(
      `Translated to ${Object.keys(languagesToTranslate).length} languages`,
    )
