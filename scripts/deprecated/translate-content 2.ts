...
async function extractLegalContent(filePath: string): Promise<{
  ...
  if (tocMatch) {
    try {
      ...
      const tocString = tocMatch[1].replace(/\n/g, '').replace(/\s+/g, ' ')
...
async function translateLegalDocuments() {
  ...
  for (const doc of legalDocs) {
    ...
    try {
      ...
      await fs.writeFile(enPath, JSON.stringify(content, null, 2), 'utf-8')
...
async function main() {
  ...
  try {
    ...
    if (usage.character) {
      ...
      console.log(
        `📊 Usage: ${usage.character.count.toLocaleString()} / ${usage.character.limit.toLocaleString()} characters`,
      )
