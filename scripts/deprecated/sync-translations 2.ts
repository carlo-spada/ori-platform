...
function notifyError(error: Error | unknown, context: string) {
  ...
  if (errorMessage?.includes('Authorization failure')) {
    ...
    console.error(
      '   - Current key type: ' + (isFreeKey ? 'FREE (ends with :fx)' : 'PRO'),
    )
    console.error('\n📝 To Fix:')
    console.error(
    ...
    console.error(
      '   4. For GitHub Actions: Update the secret in repository settings',
    )
  } else if (
    ...
    console.error(
      '   - Consider upgrading your plan or waiting for the next billing cycle',
    )
...
async function translateMissingKeys(
  ...
  try {
    ...
    console.log(
      `  📝 ${namespace} (${targetLang}): Translating ${missingKeys.length} missing keys...`,
    )
      ...
      if (typeof sourceValue === 'string') {
        ...
        if (translated % 5 === 0 || translated === missingKeys.length) {
          process.stdout.write(
            `\r    → Progress: ${translated}/${missingKeys.length} keys translated`,
          )
        }
    ...
    console.log(
      `  ✅ ${namespace} (${targetLang}): ${translated} keys translated and saved`,
    )
    return translated
  } catch (error) {
  ...
  try {
    ...
    if (usage.character) {
      ...
      if (percent >= 90) {
        ...
        console.warn(
          `   Only ${(usage.character.limit - usage.character.count).toLocaleString()} characters remaining`,
        )
    ...
    if (finalUsage.character && usage.character) {
      ...
      console.log(
        `📝 Characters used in this session: ${used.toLocaleString()}`,
      )
    }
  } catch (error: Error | unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error)
